...
{
	//cprintf("page_alloc : page_free_list %08x\n",page_free_list);
	if (!page_free_list) return NULL;
	struct PageInfo* ret = page_free_list;
	page_free_list = page_free_list->pp_link;
	//cprintf("page_free_list in page_alloc:%08x\n",page_free_list);
	ret -> pp_link = NULL;
...
page_free(struct PageInfo *pp)
{
	...
	page_free_list = pp ;
	//cprintf("page_free_list in page_free*************** %08x\n",page_free_list);
}
//
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!page_exist)
	{
		...
		else
		{
			...
			*pgdir_entry = (pte_t)page2pa(pginfo) | PTE_U | PTE_W | PTE_P;
			// cprintf("**** *pgdir_entry% %08x\n",*pgdir_entry);
			// return pgdir_entry;
		}
	}
	// what if pre_t present bit is 0?
	// 注意PTR_ADDR返回的是物理地址，而一个pointer应该指向一个虚拟地址。
	//
	...
	if (!page_exist)
	{
		...
		*pgtable_entry = page2pa(pginfo) | PTE_U | PTE_W | PTE_P;
	}
	*/
...
check_kern_pgdir(void)
{
	...
	//cprintf("711 check: %08x %08x\n",check_va2pa(pgdir,KSTACKTOP-PTSIZE),KSTACKTOP-PTSIZE);
	assert(check_va2pa(pgdir, KSTACKTOP - PTSIZE) == ~0);
	// check PDE permissions
	// cprintf("PDX(KERNBASE):%08x\n",PDX(KERNBASE));
...
check_page(void)
{
	...
	page_free(pp0);
	// cprintf("page_free_list after free pp0:%08x\n",page_free_list);
	assert(page_insert(kern_pgdir, pp1, 0x0, PTE_W) == 0);
